...
y_pred3 = np.where(y_pred == 3, y_pred2, y_pred)
df = pd.DataFrame({'y_test': y_test, 'y_pred': y_pred3})

# Drop all rows except y_pred == 3
df = df[df['y_pred'] == 3]
...
df2 = df2.drop(['Date', 'y_test', 'y_pred', 'Div','FTAG','FTHG','H', 'A', 'D','Y', 'HomeTeam', 'AwayTeam'], axis=1)
...
df4['win'] = np.where(df4['y_test'] == 3, df4['H'] - 1, -1)
